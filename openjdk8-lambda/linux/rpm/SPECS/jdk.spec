...
#
# OpenJDK 1.8.0 with Lambda packaging (32/64bits)
#
...
Name:    jdk-%{javaver}-lambda-%{origin}-%{jvm_version}-%{jdk_model}
%define jdkdir          %{_jvmdir}/jdk-%{javaver}-lambda-%{origin}-%{jdk_model}-%{jvm_version}
%else
# Name contain jdk + Version + Origin + Architecture (32/64) -> Ops mode
Name:    jdk-%{javaver}-lambda-%{origin}-%{jdk_model}
%define jdkdir          %{_jvmdir}/jdk-%{javaver}-lambda-%{origin}-%{jdk_model}
%endif
