...
#
# OpenJDK 1.8.0 with Jigsaw packaging (32/64bits)
#
...
Name:    jre-%{javaver}-jigsaw-%{origin}-%{jvm_version}-%{jdk_model}
%define jredir          %{_jvmdir}/jre-%{javaver}-jigsaw-%{origin}-%{jdk_model}-%{jvm_version}
%else
# Name contain jdk + Version + Origin + Architecture (32/64) -> Ops mode
Name:    jre-%{javaver}-jigsaw-%{origin}-%{jdk_model}
%define jredir          %{_jvmdir}/jre-%{javaver}-jigsaw-%{origin}-%{jdk_model}
%endif
