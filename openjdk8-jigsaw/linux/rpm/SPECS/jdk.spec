...
Name:    jdk-%{javaver}-jigsaw-%{origin}-%{jvm_version}-%{jdk_model}
%define jdkdir          %{_jvmdir}/jdk-%{javaver}-jigsaw-%{origin}-%{jdk_model}-%{jvm_version}
%else
# Name contain jdk + Version + Origin + Architecture (32/64) -> Ops mode
Name:    jdk-%{javaver}-jigsaw-%{origin}-%{jdk_model}
%define jdkdir          %{_jvmdir}/jdk-%{javaver}-jigsaw-%{origin}-%{jdk_model}
%endif
