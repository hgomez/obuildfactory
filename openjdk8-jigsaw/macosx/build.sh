...
function test_build()
{
  if [ -x $IMAGE_BUILD_DIR/jdk-base-image/bin/java ]; then
    $IMAGE_BUILD_DIR/jdk-base-image/bin/java -version
  else
    echo "can't find java into JDK $IMAGE_BUILD_DIR/jdk-base-image, build failed" 
    exit -1
   fi

   if [ -x $IMAGE_BUILD_DIR/jre-base-image/bin/java ]; then
     $IMAGE_BUILD_DIR/jre-base-image/bin/java -version
   else
     echo "can't find java into JRE $IMAGE_BUILD_DIR/jre-base-image, build failed" 
     exit -1
    fi
}
