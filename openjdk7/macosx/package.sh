...
JDK_DST_BUNDLE_DIRNAME=1.7.0u.jdk
JDK_BUNDLE=$OBF_DROP_DIR/$OBF_PROJECT_NAME/j2sdk-bundle-$OBF_BASE_ARCH-$OBF_BUILD_NUMBER-$OBF_BUILD_DATE.tar.bz2

JRE_ORG_BUNDLE_DIRNAME=jre1.7.0.jre
JRE_DST_BUNDLE_DIRNAME=1.7.0u.jre
JRE_BUNDLE=$OBF_DROP_DIR/$OBF_PROJECT_NAME/j2re-bundle-$OBF_BASE_ARCH-$OBF_BUILD_NUMBER-$OBF_BUILD_DATE.tar.bz2
...
function build_dmg()
{
  ...
  if [ -f $BUNDLE_FILE ]; then
    ...
    mv $FILE_NAME-$OBF_BASE_ARCH-$PACKAGE_NAME-$OBF_BUILD_NUMBER-$OBF_BUILD_DATE.dmg $OBF_DROP_DIR/$OBF_PROJECT_NAME/$OBF_DISTRIBUTION/$OBF_RELEASE_VERSION/$OBF_BASE_ARCH
...
build_dmg jdk $JDK_BUNDLE $JDK_ORG_BUNDLE_DIRNAME $JDK_DST_BUNDLE_DIRNAME 1.7.0 1.7.0.jdk OpenJDK-OSX-1.7

build_dmg jre $JRE_BUNDLE $JRE_ORG_BUNDLE_DIRNAME $JRE_DST_BUNDLE_DIRNAME 1.7.0 1.7.0.jre OpenJDK-OSX-1.7
